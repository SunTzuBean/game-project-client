...
void ScriptManager::runScript(std::string functionName, std::string type, int arg_count, ...)
{
	if (this->scriptLoaded)
	{
		...
		for (int i = 0; i < arg_count; i++)
		{
			...
			if (type == T_PLAYER) {
				dukglue_push(this->ctx, va_arg(args, Player*));
			}
			else if (type == T_INT) {
				dukglue_push(this->ctx, va_arg(args, int));
			}
			else if (type == T_EVENT) {
				dukglue_push(this->ctx, va_arg(args, EventManager*));
				dukglue_push(this->ctx, va_arg(args, Event*));
				dukglue_push(this->ctx, va_arg(args, Variant*));
				dukglue_push(this->ctx, va_arg(args, Variant*));
				break;
			}
